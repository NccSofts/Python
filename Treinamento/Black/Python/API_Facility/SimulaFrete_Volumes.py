...
colunas = ['Order', 'ChavePedido', 'Transportadora', 'CNPJTransp', 'ValorFrete', 'Prazo', 'CNPJCliente', 'RazaoSocial', 'MenorPreco',
           'DataConsulta', 'Cubagem','QtdVolumes', 'PesoTotal']
lst = []
...
while 1:
    q = status_check_cursor.execute('select status from PowerBIv2..RunningStatus').fetchone()
    if q[0] == 0:
        break
print('')

cursor.execute("EXEC Pedidos_SimuleFreteNFs")
while 1:
    q = status_check_cursor.execute('select status from PowerBIv2..RunningStatus').fetchone()
    if q[0] == 0:
        break
...
for row in df.itertuples():
    ...
    if erro == 'errorMessage':
        ...
        lst.append([cont, Pedido, str(j), '', '', '', CNPJDestino, RazaoSocial, '', datai, 0, 0, 0])
        lin = lin + 1
        contador = contador + 1
        ...
        for items in j['pricingHaulerList']:
                ...
                try:
                    ...
                    frete = j["pricingHaulerList"][cont]['priceFreight']
                    transportadora = str(j["pricingHaulerList"][cont]['tradeHauler'])
                    cont += 1

                except KeyError:
                    print('ERRO AO SIMULAR O PEDIDO', Pedido, RazaoSocial, ' | QTDVOLUMES', QtdVolumes, ' | CUBAGEM:', Cubagem)
                    cont += 1
        print(contador, 'SIMULANDO O FRETE DO PEDIDO', Pedido, RazaoSocial, ' | QTDVOLUMES', QtdVolumes, ' | CUBAGEM:', Cubagem , ' | Transp.:', transportadora, ' | Frete:', frete )
        lin = lin + 1
        contador = contador + 1
...
while 1:
    q = status_check_cursor.execute("select RunningStatus from PowerBIv2..Acompanhamento_Rotinas_SQL_Python where Script_Procedure = 'SimulaFrete_Volumes'").fetchone()
    if q[0] == 0:
        break
