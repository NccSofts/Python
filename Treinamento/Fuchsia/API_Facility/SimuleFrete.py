...
colunas = ['Order', 'Pedido', 'Transportadora', 'CNPJTransp', 'ValorFrete', 'Prazo', 'CNPJCliente', 'RazaoSocial', 'MenorPreco','DataConsulta']
lst = []
...
df = pd.read_sql_table('vBaseSimuleFrete', engine)
totalreg = str(len(df))
        ...
        for items in j['pricingHaulerList']:
                ...
                try:
                    lst.append([cont, Pedido, str(j["pricingHaulerList"][cont]['tradeHauler']), j["pricingHaulerList"][cont]['documentMainHauler'], j["pricingHaulerList"][cont]['priceFreight'],
                               j["pricingHaulerList"][cont]['deliveryTimeFreight'], CNPJDestino, RazaoSocial, '', datai])
                    cont += 1
                except KeyError:
...
df2.to_sql(
    name='Tabela_SimuleFrete_Temp', # database table name
    con=engine,
    if_exists='replace',
    index=False,
)
