...
with open("C:\Python\Comercial\Acompanhamento_DL.html", "a") as file:
    file.write('<span style="text-decoration: underline;">RESUMO (Valores sem impostos):</span>')

resumo = pd.read_sql_table("Novo_Consolidado_Industrias_SemImpostos", engine)
...
pendentes_entrega = pd.read_sql("SELECT * FROM PowerBiv2..Acompanhamento_Industrias_Base_v2 WHERE Status1 IN ('Pendente Entrega em atraso') AND Industria <> 'Positivo'", engine)
resumo_pendentes = pd.pivot_table(pendentes_entrega, values=['Valor_Sem_Impostos'], aggfunc='sum',
...
a_faturar = pd.read_sql("SELECT * FROM PowerBiv2..Acompanhamento_Industrias_Base_v2 WHERE Status1 IN ('A Faturar') AND StatusNaoAplicavel = 'Pendente Entrega'", engine)
resumo_faturar = pd.pivot_table(a_faturar, values=['Valor_Sem_Impostos'], aggfunc='sum',
...
# PEDENTES DE FATURAMENTO
envio_fidc = pd.read_sql("SELECT * FROM PowerBiv2..Acompanhamento_Industrias_Base_v2 WHERE StatusNaoAplicavel IN ('Pendente de envio FIDC')", engine)
# resumo_fidc = pd.pivot_table(envio_fidc, values=['Valor_Sem_Impostos'], aggfunc='sum',
#                                   index=['Industria', 'AnoMes_Ped', 'Pedido', 'Data_Pedido', 'Razao_Social'], margins=True,
#                                   margins_name='Total', fill_value=0)
...
if diasemana < 5:
    ...
    titulo = str('Acompanhamento Consolidado de Serviços (Sem Impostos) - Posição em: ' + str(data_pt_br))

    # titulo = 'Acompanhamento Processos DL (Sem Impostos) - Posição em ' + str(data_pt_br)
    # sender_email = "credito@maisproxima.com.br"
    user_login = 'bi@maisproxima.com'
    # receiver_email = ['anderson.souza@maisproxima.com.br']
    receiver_email = ['igor.chaves@maisproxima.com.br', 'Simone Dario <simone.dario@maisproxima.com.br>', 'anderson.souza@maisproxima.com.br']
    ...
    cursor.execute("EXEC POWERBIV2..PROC_RUN_ROTINAS 'Novo Consolidado Industrias sem impostos'")
