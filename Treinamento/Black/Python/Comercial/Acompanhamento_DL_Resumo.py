...
texto = """\
        ...
        <h1 style="color: #5e9ca0;"><strong style="color: #000000; font-size: 14px;">Abaixo temos um resumo da Operação DL.</strong></h1>
...
resumo = pd.read_sql_table('Acompanhamento_DL_Resumo', engine)
...
lista_dl_faturar = """\
                            ...
                            WHERE Industria = 'DL' AND [Status-iMPX] LIKE '41-%' AND Data >= FORMAT(DATEADD(MONTH, -4, GETDATE()), 'yyyy-MM-01')
                               AND [Status-DL NF] NOT IN ('80-EXTRAVIO\CANCELADA','50-CANCELADO - NF', '70-DEVOLVIDO')             
...
resumo_dl = pd.pivot_table(lista_dl_pd, values=['Total Pedido'], aggfunc='sum', index=['Ano', 'Mes', 'Dia', 'Pedido', 'Razao Social'], margins=True, margins_name='Total', fill_value=0)
resumo2_dl = pd.pivot_table(lista_dl_pd, values=['Total Pedido'], aggfunc='sum', index=['Ano', 'Mes'], margins=True, margins_name='Total', fill_value=0)

resumo_dl.to_html('C:\Python\Comercial\Resumo1.html')
resumo2_dl.to_html('C:\Python\Comercial\Resumo2.html')

resumo_sql = open('C:\Python\Comercial\Resumo2.html', 'r')
html = resumo_sql.read()
...
lista_dl_faturar = """\
                        ...
                        WHERE Industria = 'DL' AND [Status-iMPX] LIKE '43-%' --AND Data >= FORMAT(DATEADD(MONTH, -4, GETDATE()), 'yyyy-MM-01')
...
resumo_dl = pd.pivot_table(lista_dl_pd, values=['Total Pedido'], aggfunc='sum', index=['Ano', 'Data', 'Pedido', 'DataPedido', 'Razao Social'], margins=True, margins_name='Total', fill_value=0)

resumo_dl.to_html('C:\Python\Comercial\Resumo3.html')

resumo_sql = open('C:\Python\Comercial\Resumo3.html', 'r')
html = resumo_sql.read()
...
resumo_sql = open('C:\Python\Comercial\Resumo4.html', 'r')
html = resumo_sql.read()
...
if diasemana < 5:
    ...
    titulo = str('Acompanhamento DL - Posição em: ' + str(data_pt_br))
