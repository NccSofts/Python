...
query_nfs = """\
                ...
                FROM Tabela_Rentabilidade_BI
               WHERE TipoNF = 'Venda' AND TipoFrete in ('C', 'F') AND Data >= '2019-09-01' AND
	                 IIF(DataExpedicao IS NULL, 'A',IIF(DataPrevisaoEntrega IS NULL, 'B', IIF(DataEntrega IS NULL, 'C','N'))) IN ('A', 'C')
                GROUP BY Numero, CAST(LEFT(Numero, 6) AS INT), TipoFrete, ForCli,  Data 
                ORDER BY NF Desc
            """
