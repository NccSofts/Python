...
cursor.execute("EXEC Datawarehouse..Pedidos_SimuleFrete_FOB")
while 1:
...
df = pd.read_sql_table('vBaseSimuleFrete_FOB', engine)
totalreg = str(len(df))
...
for row in df.itertuples():
    ...
    querystring = {"originZipcode": CEPOrigem, "destinationZipcode": CEPDestino, "documentClient": CNPJOrigem,
                   ...
                   "totalWeight": PesoTotal.replace(',','.'), "cubicMeter": Cubagem.replace(',','.'), "totalVolumes": QtdVol_int, "orderDate": DataPedido, "haulersLimit": "3",
                   "sortBy": "lowest_price"}
...
df2.to_sql(
    name='Tabela_SimuleFrete_Temp_FOB', # database table name
    con=engine,
    if_exists='replace',
    index=False
)
...
cursor.execute("EXEC Datawarehouse..Ajustes_SimuleFrete_FOB")
while 1:
    q = status_check_cursor.execute("select RunningStatus from PowerBIv2..Acompanhamento_Rotinas_SQL_Python where Script_Procedure = 'SimuleFrete_FOBv2'").fetchone()
    if q[0] == 0:
        break
